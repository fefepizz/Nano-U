...
if __name__ == '__main__':
    ...
    nano_u_tflite_info = {
        'model': interpreter,
        'type': 'tflite',
        'path': nano_u_tflite_path,
        'pytorch_params': get_model_parameters(nano_u_info),
        'name': 'Nano_U_int8'
    }
    ...
    results['Nano_U'] = benchmark_model(nano_u_info, dataloader, DEVICE,
                                       store_examples=True, num_examples=NUM_EXAMPLES)

    # Benchmark Nano_U TFLite
    print("\n--- Starting Benchmark: Nano_U_int8 (TFLite) ---")
    results['Nano_U_int8'] = benchmark_model(nano_u_tflite_info, dataloader, DEVICE,
                                            store_examples=True, num_examples=NUM_EXAMPLES)
    ...
    print("="*90)
    print(f"{'Model':<15} | {'Parameters (M)':<15} | {'Size (KB)':<10} | {'IoU':<8} | {'CE Accuracy':<12} | {'Time (ms)':<12}")
    print("-" * 90)
    for name, res in results.items():
        print(f"{name:<15} | {res['params_m']:<15.2f} | {res['size_kb']:<10.2f} | {res['avg_iou']:<8.3f} | {res['avg_ce_accuracy']:<12.3f} | {res['avg_time_ms']:<12.2f}")
    print("-" * 90)
